...
xlim([300 340]);
% A % Shifted loner wavelengths by ~1 channel 
% B % Shifted to shorter wavelengths 5 channels (close to limit permitted in DOAS fitting)
% C % Shifted to longer wavelengths -8 channels 
% A 
s2icropA = s2i(1:end-1,1); 
wHgCropA = wHg(2:end,1); 
s1icropA = s1i(2:end,1); 
plot(wHgCropA, s2icropA, '-b'); 
% B 
s2icropB = s2i(6:end,1); 
wHgCropB = wHg(1:end-5,1);
s1icropB = s1i(1:end-5,1); 
plot(wHgCropB, s2icropB, '-r'); 
% C
s2icropC = s2i(1:end-8,1); 
wHgCropC = wHg(9:end,1);
s1icropC = s1i(9:end,1); 
...
valC = nanmean(degC, 'all');
STDC = std(degC(:));
degX = degX(indxEFh:indxEFl,indxEWl:indxEWu); 
