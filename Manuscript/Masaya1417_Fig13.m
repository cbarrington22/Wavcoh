...
toAnalyse = '/Users/charlotteb/Documents/Data/D2J2375Florian/D2J2375_140319_1417_0/'; % Directory to full full instrument scan
...
for i = cSpec(1:15) % 1:cSpec 
    ...
    xlim([min(wHg) max(wHg)]); ylim([min(fwc) max(fwc)]); 
    box on
    Ang = string(scanAngle(:, i));
    title(sprintf('%s°', Ang));
    wcohEx = abs(wcoh(indxEFh:indxEFl,indxEWl:indxEWu)); 
...
% rectangle('Position', posE,'LineWidth', 0.5,'LineStyle', '-', 'EdgeColor', 'k');
...
p = plot(x, 1-masaya1417, '-ok'); hold on; % Plots minimum wavelet coherend against scan angle 
p.MarkerFaceColor = 'k'; 
% xlabel('Scan angle (°)');
...
% 
subplot(2,2,3)
blMSWC = max(masaya1417_sum); 
dmasaya1417_sum = masaya1417_sum-blMSWC; 
